...
def generate_gpx(gps_data, out_file, in_file):
...
def write_if_gps_data(gps_data, out_file, in_file):
    """ checks if the gps_data is there and then generates the gpx """
    if gps_data:
        gpx = generate_gpx(gps_data, out_file, in_file)
...
def main():
    ...
    if multiple:
        for in_file in in_files:
            ...
            write_success = write_if_gps_data(gps_data, out_file, in_file)
        ...
        success = write_if_gps_data(gps_data, out_file, in_file)
    if not success:
        print("Failure!")
        sys.exit(1)
    else:
